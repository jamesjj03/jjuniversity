...
| `public/atlas-world/layers/*.equal-earth.webp` | Bounded 2400 × 1300 population-density and relief overviews aligned to the map viewBox |
| `public/atlas-world/layers/population-density-2025/{regional,country}/` | Viewport-loadable source-detail population tiles, at most 1200 × 1300 pixels each |
...
The locked GHSL input is the official `GHS_POP_E2025_GLOBE_R2023A_54009_1000_V1_0.zip` (323,340,844 bytes, SHA-256 `cd630f51ac65dff2a0c7ad252333bbb20c5cf9de4d85eef04e22ef3699d80c95`, CC BY 4.0). Its cells store estimated people per 1 km equal-area cell. Atlas independently area-averages the original source into a 2400 × 1300 Equal Earth overview and tiled 9600 × 5200 / 19200 × 10400 detail levels, applies the same explicit `log1p` color/opacity scale, and makes zero/no-data pixels transparent. Exact derivative byte counts and checksums live in the geography pack. Display pixels aggregate source cells and are not exact per-pixel measurements. See [Phase 2.5 cartography](atlas-phase25-cartography.md) for resolution, alignment, and memory details.

### Physical context

- **Relief:** [Natural Earth 1:50m Manual Shaded Relief 3.3.0](https://www.naturalearthdata.com/downloads/50m-raster-data/50m-manual-shaded-relief/), public domain. Atlas bilinear-warps the 10,800 × 5,400 WGS84 grayscale source to the spherical Equal Earth formula used by its SVG, clips it to the sphere, and applies a globally recorded 1.45 shadow-contrast transfer. Styling opacity stays outside the raster. It is cartographic shading based on elevation reference data, not an elevation measurement layer. Exact asset size and checksum are in the geography pack.
...
- The 2400 × 1300 overview is backed by independently source-generated regional/country tiles. Their highest level approaches two projected kilometres per display pixel; neither further zoom nor interpolation provides street-level population detail.
