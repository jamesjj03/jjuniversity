-- Operator-only rollback for jju_tacos_track_reconciliation_2026_08_28.sql.
--
-- This is not part of the normal forward migration sequence. Run it only to
-- restore the exact three pre-reconciliation QA references preserved by
-- tacos-approved-alternatives-10-13-16-v1. It refuses partial states, verifies
-- the immutable audit snapshot and all six Storage objects, preserves the
-- audit row, and never changes Storage or publication state.
...
begin
  if to_regclass('public.jju_audio_track_reconciliation_audit') is null then
    raise exception 'The Tacos reconciliation audit table does not exist.'
      using errcode = '55000';
  end if;
end;
$require_audit_table$;
...
create temporary table jju_tacos_reconciliation_rollback_tracks (
...
insert into jju_tacos_reconciliation_rollback_tracks values
  (
...
create temporary table jju_tacos_reconciliation_rollback_objects (
...
insert into jju_tacos_reconciliation_rollback_objects values
  ('previous', '22a844c9-ba50-4d9d-bf90-dccbc41677e1'::uuid, 'audiobooks', 'everything-i-touch-turns-to-tacos/standard/10-992cecfdefaa74d2.mp3', 5489480, 'audio/mpeg', '84c0b1f97fb7a34626392b9d05778bb4'),
  ('target', '96d5d79c-78c6-4ab0-8819-ff320eb2737b'::uuid, 'audiobooks', 'everything-i-touch-turns-to-tacos/standard/10-0a69afb8a56a0b6e.mp3', 5529152, 'audio/mpeg', '1252e630e7e59fac8e394b97634a7329'),
  ('previous', '742d30ef-e9e8-4408-881a-882af1420e77'::uuid, 'audiobooks', 'everything-i-touch-turns-to-tacos/standard/13-ebbf68878706555c.mp3', 907364, 'audio/mpeg', '518b921828f08d829483e3af22ff3f42'),
  ('target', '0e5c2c82-6121-45d9-9851-33bea1263359'::uuid, 'audiobooks', 'everything-i-touch-turns-to-tacos/standard/13-b0646c8581ffc3e4.mp3', 919892, 'audio/mpeg', '00eb978682a016e39394ed395fe418d3'),
  ('previous', '093e228d-e4d6-4418-8f34-df91c3691678'::uuid, 'audiobooks', 'everything-i-touch-turns-to-tacos/standard/16-743ea9ec251007bb.mp3', 1775972, 'audio/mpeg', '082f9b4f3fb128a26d1c64c976e24efc'),
  ('target', '83f31c60-36e2-42ea-a710-aab815e4b878'::uuid, 'audiobooks', 'everything-i-touch-turns-to-tacos/standard/16-5981f85def7c4f7c.mp3', 1806248, 'audio/mpeg', 'bea0b1749701deff6b8ce5d3da6c939c');
...
begin
  if not exists (
    ...
    raise exception 'The exact Tacos reconciliation snapshot is missing or invalid.'
      using errcode = '55000';
  end if;
  ...
  join jju_tacos_reconciliation_rollback_tracks expected
    on snapshot->>'id' = expected.track_id::text
   and (snapshot->>'position')::integer = expected.position
   and snapshot->>'title' = expected.title
   and snapshot->>'section_key' = expected.section_key
   and (snapshot->>'required_for_submission')::boolean = expected.required_for_submission
   and snapshot->>'storage_bucket' = expected.storage_bucket
   and snapshot->>'storage_path' = expected.old_storage_path
   and snapshot->>'mime_type' = expected.mime_type
  ...
  join jju_tacos_reconciliation_rollback_tracks expected
   ...
   and manifest->>'storage_bucket' = expected.storage_bucket
   and manifest->>'storage_path' = expected.new_storage_path
   and manifest->>'mime_type' = expected.mime_type
  ...
  join jju_tacos_reconciliation_rollback_objects expected
  ...
  then
    raise exception 'The Tacos audit snapshot does not match the exact reversible states.'
      using errcode = '55000';
  end if;
  ...
  if v_edition_status <> 'qa' or v_edition_published_at is not null then
    raise exception 'Rollback refuses to alter published or non-QA Tacos rows.'
      using errcode = '55000';
  end if;
  ...
  if v_track_count <> 16 or v_qa_track_count <> 16 then
    raise exception 'Expected exactly 16 unpublished QA tracks for rollback.'
      using errcode = '55000';
  end if;
  ...
  from jju_tacos_reconciliation_rollback_tracks expected
  ...
  if v_new_matches = 3 and v_old_matches = 0 then
    ...
    then
      raise exception 'The reconciled Tacos state drifted before rollback.'
        using errcode = '55000';
    end if;
  elsif v_old_matches = 3 and v_new_matches = 0 then
    ...
    then
      raise exception 'The rolled-back Tacos state drifted.'
        using errcode = '55000';
    end if;
  else
    raise exception 'Tracks 10, 13, and 16 are partial or differ from both reversible states.'
      using errcode = '55000';
  end if;
  ...
  if exists (
    ...
    raise exception 'Narrator workflow rows now reference Tacos; rollback requires manual review.'
      using errcode = '55000';
  end if;

  -- Hold the exact private-bucket settings until commit so rollback cannot
  -- race a concurrent Storage configuration change.
  perform bucket.id
  ...
  if not found then
    raise exception 'The private audiobooks bucket settings drifted before rollback.'
      using errcode = '55000';
  end if;
  ...
  from jju_tacos_reconciliation_rollback_objects expected
  ...
  if v_object_matches <> 6 then
    raise exception 'A previous or replacement Tacos Storage object drifted before rollback.'
      using errcode = '55000';
  end if;
  ...
  if exists (
    ...
    raise exception 'A previous Storage path is now owned by another track.'
      using errcode = '55000';
  end if;
end;
$preflight$;
...
set storage_path = expected.old_storage_path,
    file_size_bytes = expected.old_file_size_bytes,
    duration_seconds = expected.old_duration_seconds,
    sha256 = expected.old_sha256
from jju_tacos_reconciliation_rollback_tracks expected
where track.id = expected.track_id
...
begin
  if not exists (
    ...
    raise exception 'The Tacos edition did not return to the exact unpublished QA baseline.'
      using errcode = '55000';
  end if;
  ...
  if v_old_matches <> 3 then
    raise exception 'Tracks 10, 13, and 16 did not roll back exactly.'
      using errcode = '55000';
  end if;
  ...
  if v_track_count <> 16
    ...
    or v_total_bytes <> 49834256
    or v_track_seconds <> 1246
    or v_track_core_fingerprint is distinct from
      'f91083c863fc8b17801a8a2bac6645087baf0d00618a9779a2685919eeb85ffc'
  then
    raise exception 'The rolled-back Tacos package differs from the exact QA baseline.'
      using errcode = '55000';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The private audiobooks bucket settings changed during rollback.'
      using errcode = '55000';
  end if;
  ...
  if v_object_matches <> 6 then
    raise exception 'A pinned Tacos Storage object changed during rollback.'
      using errcode = '55000';
  end if;
end;
$postcheck$;
